...
#
# 解决：把上述 3 类缓存 key 全清，让下次访问重读 DB / 重反射。
#
# 用法：
...
echo "[djs-post-init] 1/3 清 ruoyi 字典 NullValue 缓存（*:sys_dict）..."
...
echo "[djs-post-init] 2/3 清 djs 字典聚合缓存历史残留（*djs:dict*，实时算重构前的旧快照）..."
docker exec "$REDIS_CONTAINER" redis-cli -a "$REDIS_PASSWORD" --no-auth-warning \
  --scan --pattern '*djs:dict*' \
  | xargs -I {} docker exec "$REDIS_CONTAINER" redis-cli -a "$REDIS_PASSWORD" --no-auth-warning DEL {} \
  || true

echo "[djs-post-init] 3/3 清客户端授权配置缓存（*sys_client*，登录态时长取自它）..."
docker exec "$REDIS_CONTAINER" redis-cli -a "$REDIS_PASSWORD" --no-auth-warning \
