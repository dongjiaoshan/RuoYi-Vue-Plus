...
VALUES (1001, 'DJS-MAIN', '东角山主场', 1, 'system', NOW())
...
CREATE TABLE t_md_store (
  ...
  store_type      VARCHAR(16)  NOT NULL DEFAULT 'direct' COMMENT '门店类型 字典 coop_status: direct=直营 / franchise=加盟',
  business_status TINYINT      NOT NULL DEFAULT 1 COMMENT '经营状态 1=合作中 0=已终止',
  address         VARCHAR(255) NULL COMMENT '门店地址',
  contact_name    VARCHAR(32)  NULL COMMENT '联系人',
  contact_phone   VARCHAR(20)  NULL COMMENT '联系电话',
  warehouse_id    BIGINT       NULL COMMENT '关联仓库 ID（V1 默认一店一仓；推断字段）',
  settle_type     VARCHAR(16)  NULL COMMENT '结算方式 字典 settle_type: cash/monthly/quarterly',
  create_by       VARCHAR(64)  DEFAULT '' COMMENT '创建人',
  create_time     DATETIME     NULL COMMENT '创建时间',
  update_by       VARCHAR(64)  DEFAULT '' COMMENT '更新人',
  update_time     DATETIME     NULL COMMENT '更新时间',
...
CREATE TABLE t_md_supplier (
  ...
  supplier_type   VARCHAR(32)  NOT NULL COMMENT '供应商类型 字典 settle_type: medicine/breeding_pig/feed/seed',
  contact_name    VARCHAR(32)  NULL COMMENT '联系人',
  contact_phone   VARCHAR(20)  NULL COMMENT '联系电话',
  address         VARCHAR(255) NULL COMMENT '地址',
  business_status TINYINT      NOT NULL DEFAULT 1 COMMENT '合作状态 1=合作中 0=已终止 字典 version_desc',
  settle_type     VARCHAR(16)  NULL COMMENT '结算方式 字典 shop_type',
