...
/**
 * 当月生产统计表（率 / 窝均 10 行）VO（FIX-MGMT-MP-BRD-001，原型"种猪 tab·当月生产统计表 10 行"，#7.8）。
 *
 * <p>原型当月生产统计表 = 10 行率/窝均指标 × (当月 vs 上月)。区别于现有
 * {@code /monthly-comparison}（7 行头数快照）：本端点输出原型要求的 10 行率/窝均口径，实时算。</p>
 *
 * <p>10 行口径：分娩率 / 配怀率 / 断配间隔 / 返空流头数 / 平均非生产天数NPD / 总产仔数 /
 * 窝均总产仔 / 窝均活仔 / 窝均断奶 / 产房损失率。复用 BreedingAnnual 同套公式（#7.1-7.5）。</p>
 *
 * @author djs
