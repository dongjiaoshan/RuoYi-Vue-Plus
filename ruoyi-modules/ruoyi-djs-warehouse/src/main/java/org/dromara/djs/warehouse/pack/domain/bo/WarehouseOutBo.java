 ...
 * 出库不发往门店，而是记「出库去向」（字典 {@code djs_stock_out_dest}，如矿山 / 厨房 / 大冶门店 / 个人），
 * 出库方式=后台出库。复用 {@code submitWhiteBarOut} 收尾范式，但 <b>不绑门店、不扣门店需求</b>。
 * admin row2：该出库去向码值同时落 {@code t_warehouse_pig_cut_record.out_dest}。</p>
 *
 * @author djs
 * @since row17
 */
...
public class WarehouseOutBo {
    ...
    /**
     * 出库去向（字典 {@code djs_stock_out_dest} 的 dict_value；admin row2 落 cut_record.out_dest）。
     */
