...
public class StoreDailyLedgerCandidateVo implements Serializable {
    ...
    /**
     * 产品对应原材料 {@code product_material} 的计量单位（如 kg）；仅 pork 类目行有值。
     * 前端据此把猪肉产品行按重量口径展示（KG 保留 3 位），其余行回落 {@link #productUnit}。
     */
     ...
     * <p>前端「新增当日盘点」按此分 3 个 TAB 展示。归属：产品在 {@code djs_white_bar_return_product} 字典
     * 或 {@code belong_type='pork'} → pork；{@code belong_type='vegetable'} → veg；
