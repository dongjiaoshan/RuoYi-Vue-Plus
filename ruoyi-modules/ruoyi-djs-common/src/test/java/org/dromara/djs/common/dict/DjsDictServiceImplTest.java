 ...
 * <p>覆盖：</p>
 * <ul>
 *   <li>queryAllDjsTypes：遍历 35 个 dict_type 全部调 ruoyi 一次</li>
 ...
 *   <li>常量加载：DictTypeConstants 反射出来正好 35 项</li>
 * </ul>
 *
...
class DjsDictServiceImplTest {
    ...
    @Test
    @DisplayName("DictTypeConstants 反射出来正好 35 项")
    ...
    @Test
    @DisplayName("queryAllDjsTypes：遍历 35 个 dict_type 全部调 ruoyi 一次")
    void queryAllDjsTypes_callsRuoyiOncePerType() {
        ...
        verify(sysDictTypeService, times(35)).selectDictDataByType(anyString());
    }

    @Test
    @DisplayName("queryFull：实时聚合算 SHA-256（64 hex），返全量 35 项")
    void queryFull_computesRealtime() {
        DjsDictFullVo vo = service.queryFull();

        assertThat(vo.getVersion()).hasSize(64).matches("[0-9a-f]{64}");
        assertThat(vo.getData()).hasSize(35);
        verify(sysDictTypeService, times(35)).selectDictDataByType(anyString());
    }
