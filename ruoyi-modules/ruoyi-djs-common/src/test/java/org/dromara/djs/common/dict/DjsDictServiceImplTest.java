 ...
 * <p>覆盖：</p>
 * <ul>
 *   <li>queryAllDjsTypes：遍历 39 个 dict_type 全部调 ruoyi 一次</li>
 ...
 *   <li>常量加载：DictTypeConstants 反射出来正好 39 项</li>
 * </ul>
 *
...
class DjsDictServiceImplTest {
    ...
    @Test
    @DisplayName("DictTypeConstants 反射出来正好 39 项（与 SYS-INIT-002 init-dict.sql 对齐）")
    void allDjsDictTypesCount39() {
    ...
    @Test
    @DisplayName("queryAllDjsTypes：遍历 39 个 dict_type 全部调 ruoyi 一次")
    void queryAllDjsTypes_callsRuoyi39Times() {
        Map<String, List<SysDictDataVo>> all = service.queryAllDjsTypes();

        assertThat(all).hasSize(39);
        assertThat(all.keySet()).contains(
            DictTypeConstants.PIG_GENDER, DictTypeConstants.PIG_BREED,
            DictTypeConstants.USER_STATUS, DictTypeConstants.WAREHOUSE_TYPE);
        verify(sysDictTypeService, times(39)).selectDictDataByType(anyString());
    }
    ...
    void queryFull_cacheMiss_writesBothKeys() {
        ...
        assertThat(vo.getData()).hasSize(39);
