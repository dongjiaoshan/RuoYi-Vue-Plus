 ...
 *
 * <p>甲方点名的四项筛选，不多不少：作物名称模糊 + 上市月份 + 下架月份 + 状态。
...
public class MarketPlanQuery implements Serializable {
    ...
    /**
     * 状态码（{@code pending / upcoming / on_sale / ending / off_shelf}，见
     * {@link org.dromara.djs.plant.common.util.DateWindowStatusCalculator}）。
     *
     * <p>状态是按日期与当天现算、不落库的派生值，SQL 里没有这一列，所以这个条件
     * <b>不下推 SQL</b>，由 service 算完状态后在内存里过滤（详见
     * {@code MarketPlanServiceImpl#queryPageList}）。</p>
     */
