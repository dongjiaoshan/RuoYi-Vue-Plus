...
**总段位**：系统底座 5000-5999 / 养殖 7000-7999 / 种植 8000-8999 / 仓库 9000-9999 / 门店+追溯+DSH 10000-10999 / **mp 权限树 11000-11999** / **运营管理 12000-12999**。养殖域 7000-7999 二级分段（D04 closing 固化）：
...
| 8300-8399 | 需求反馈（作物需求的种植端回复，与 12040 共表 t_plant_crop_demand）| V6-R153 用 8300-8302 |
...
| 9130-9144 | 有机饲喂 / 毛菜间记录 / 三期作物入库 | 9130-9137 · 9140-9143 已占 |
| 9145-9199 | 出入库月汇总（+ 入库汇总 / 出库汇总两个下钻弹窗，挂库存管理 9302）| V6-R154~R156 用 9145-9148 |
...
mp 权限树 11000-11999（**不是业务域段，别当空段拿去建业务菜单**）：

| 段 | 模块 | 当前占用 |
|---|---|---|
| 11000-11099 | 小程序权限树（通用/养殖/种植/仓库/门店 mp 权限）| DJS-PERM-MENU-001~005 |
| 11100-11999 | mp 权限树扩展预留 | — |

运营管理域 12000-12999（V6-R149 起，与养殖/种植/仓库/门店并列的第六个板块，前端 `views/djs-ops/`）：

| 段 | 模块 | 当前占用 |
|---|---|---|
| 12000 | 运营管理父菜单 | V6-R149 |
| 12010 | 农场信息目录 | V6-R149 |
| 12020-12029 | 果蔬上市计划 | V6-R151 用 12020-12022 |
| 12030-12039 | 育肥猪信息 | V6-R150 用 12030-12031 |
| 12040-12049 | 作物需求 | V6-R152 用 12040-12043 |
| 12050-12999 | 运营管理扩展预留 | — |

> 运营管理**后端不单开 maven 模块**：接口按数据归属放进已有模块（上市计划/作物需求→`ruoyi-djs-plant`、育肥猪信息→`ruoyi-djs-breed`），只有前端有独立的 `views/djs-ops/` 目录。
...
- **miniapp 按分支分环境（一个 appid，两个 robot 槽位）**：`staging` 分支 → `pnpm build:mp:staging`（`.env.staging` 打 `api-staging.dongjiaoshan.com`）→ miniprogram-ci 传 **robot=2**，版本固定 `1.0.1`，给测试员当**体验版**；`main` 分支 → 打生产后端 → 传 **robot=1**，版本 `1.0.<commit 数>`，**提审用**。微信每个 robot 只留最近一次上传，两条分支各占一个槽位、在「开发版本」里并存。**正式版仍是 Kevin 手动在微信后台提审+发布**（CI 只上传，不发布）。锚点 `code/main/miniapp/.github/workflows/upload-mp-{staging,main}.yml` 顶部注释 + `.env.staging:5`。
